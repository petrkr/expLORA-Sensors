 ...
 * the same protocol and encryption as expLORA, and then
 * enters deep sleep for 15 minutes
 * 
 * Sensor ID: 0x31 (DIY DS18B20)
 * 
 ...
 */
 ...
 /**
  * Encrypt data with the same algorithm as TI example
  */
 void encrypt_data(uint8_t *data, uint8_t data_len, uint32_t key)
 {
 ...
 void sendData() {
   ...
   // Create packet
   uint8_t packet[11]; // 10 bytes + checksum
